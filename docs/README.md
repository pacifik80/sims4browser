# Documentation Map

This file is only the documentation index. Repo-level working rules live in [AGENT.md](../AGENT.md).

## Start Here

1. [AGENT.md](../AGENT.md)
2. [Knowledge map](knowledge-map.md)
3. [Current plan](planning/current-plan.md)
4. [Big plan](planning/big-plan.md)
5. [Multi-agent workflow](operations/multi-agent-workflow.md)
6. [Tooling and scratch space](operations/tooling.md)

## Knowledge Entry Points

If the task is about repo-level TS4 render/material truth:

- [Shared TS4 material, texture, and UV pipeline](shared-ts4-material-texture-pipeline.md)
- [Material research restart guide](workflows/material-pipeline/research-restart-guide.md)
- [Build/Buy material authority matrix](workflows/material-pipeline/buildbuy-material-authority-matrix.md)
- [CAS/Sim material authority matrix](workflows/material-pipeline/cas-sim-material-authority-matrix.md)
- [Shader family registry](workflows/material-pipeline/shader-family-registry.md)
- [Skintone and overlay compositor](workflows/material-pipeline/skintone-and-overlay-compositor.md)
- [Knowledge map](knowledge-map.md)

If the task is about Sim assembly and shell behavior:

- [Sim domain roadmap](sim-domain-roadmap.md)
- [Sim body-shell contract](sim-body-shell-contract.md)
- [CAS/Sim material authority matrix](workflows/material-pipeline/cas-sim-material-authority-matrix.md)

If the task is about evidence and reverse-engineering provenance:

- [References index](references/README.md)
- [Codex wiki](references/codex-wiki/README.md)
- [External snapshots](references/external/README.md)

## Product And Architecture

- [Architecture](architecture.md)
- [Shared TS4 material, texture, and UV pipeline](shared-ts4-material-texture-pipeline.md)
- [Knowledge map](knowledge-map.md)
- [Supported types](supported-types.md)
- [Known limitations](known-limitations.md)
- [Sim domain roadmap](sim-domain-roadmap.md)
- [Sim body-shell contract](sim-body-shell-contract.md)
- [Sample data strategy](sample-data-strategy.md)
- [Risk log](risk-log.md)
- [Third-party licenses](third-party-licenses.md)
- [ADR 0001: Solution architecture](adr/0001-solution-architecture.md)
- [ADR 0002: Dependency selection](adr/0002-dependency-selection.md)

## Planning

- [Current plan](planning/current-plan.md)
- [Big plan](planning/big-plan.md)
- [Technical debt](planning/technical-debt.md)
- [Unknowns and non-goals](planning/unknowns-and-non-goals.md)

## Operations

- [Chat handoff](operations/chat-handoff.md)
- [Multi-agent workflow](operations/multi-agent-workflow.md)
- [Tooling and scratch space](operations/tooling.md)

## References

- [References index](references/README.md)
- [Codex wiki](references/codex-wiki/README.md)
- [External snapshots](references/external/README.md)

## Workflows

- [Workflows index](workflows/README.md)
- [Open game and mods](workflows/open-game-and-mods.md)
- [Export Build/Buy object](workflows/export-buildbuy-object.md)
- [Export CAS asset](workflows/export-cas-asset.md)
- [Export raw resource](workflows/export-raw-resource.md)
- [Material pipeline deep dives](workflows/material-pipeline/README.md)
- [Material research restart guide](workflows/material-pipeline/research-restart-guide.md)
- [Build/Buy material authority matrix](workflows/material-pipeline/buildbuy-material-authority-matrix.md)
- [CAS/Sim material authority matrix](workflows/material-pipeline/cas-sim-material-authority-matrix.md)
- [Shader family registry](workflows/material-pipeline/shader-family-registry.md)
- [Skintone and overlay compositor](workflows/material-pipeline/skintone-and-overlay-compositor.md)
